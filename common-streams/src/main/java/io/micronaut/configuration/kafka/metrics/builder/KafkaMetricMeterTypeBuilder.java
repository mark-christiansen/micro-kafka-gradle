    ...
    public Optional<Meter> build() {
        ...
        if (kafkaMetricMeterType.getMeterType() == MeterType.GAUGE) {
            ...
            if (metricValue instanceof Number) {
                return Optional.of(Gauge.builder(getMetricName(), () -> (Number) metricValue)
            ...
            } else if (metricValue instanceof String && !"unknown".equals(metricValue)) {
                List<Tag> tags = tagFunction.apply(kafkaMetric.metricName());
                tags.add(Tag.of(kafkaMetric.metricName().name(), (String) metricValue));
                return Optional.of(Gauge.builder(getMetricName(), () -> 0)
                        .tags(tags)
                        .description(kafkaMetricMeterType.getDescription())
                        .baseUnit(kafkaMetricMeterType.getBaseUnit())
                        .register(meterRegistry));
