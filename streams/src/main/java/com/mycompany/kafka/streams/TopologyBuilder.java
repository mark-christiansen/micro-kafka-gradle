...
public class TopologyBuilder {
    ...
    public TopologyBuilder(Map<String, Object> props,
    ...
    public Topology build(Properties streamProperties) {
        ...
        builder.stream(inputTopic, Consumed.with(org.apache.kafka.common.serialization.Serdes.Long(), serdes.createGenericSerde(false)))
                .transformValues(new StatefulTransformerSupplier("consumer"))
                .map((k, v) -> {
                    log.info("Before storing message for key {}", k);
                    return new KeyValue<>(k, v);
                })
                .toTable(getStateStore("store", org.apache.kafka.common.serialization.Serdes.Long(), serdes.createGenericSerde(false)))
                .toStream()
                .transformValues(new StatefulTransformerSupplier("producer"))
                .to(outputTopic, Produced.with(org.apache.kafka.common.serialization.Serdes.Long(), serdes.createGenericSerde(false)));
    ...
    private static class StatefulTransformer implements ValueTransformerWithKey<Long, GenericRecord, GenericRecord> {
        ...
        public GenericRecord transform(Long key, GenericRecord value) {
            log.info("After storing message for key {}", key);
